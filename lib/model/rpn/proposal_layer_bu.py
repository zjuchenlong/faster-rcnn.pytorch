...
class _ProposalLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(batch_size):
            ...
            keep = self._filter_boxes(proposals_single, min_size * im_info[i, 2])
            ...
            keep_idx_i = nms(torch.cat((proposals_single, scores_single), 1).cpu().numpy(), nms_thresh, force_cpu=not cfg.USE_GPU_NMS)
            keep_idx_i = torch.from_numpy(np.array(keep_idx_i))
            if cfg.CUDA:
                keep_idx_i = keep_idx_i.cuda()

            if post_nms_topN > 0 and len(keep_idx_i) > post_nms_topN:
