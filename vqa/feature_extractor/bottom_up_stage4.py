...
def parse_args():
    ...
    parser.add_argument('--feature_save_dir', dest='feature_save_dir',
                        default='../vqa_data/features/bottom_up_stage4')
...
if __name__ == '__main__':
    ...
    # for split_name in ['val']:
        ...
        split_feat = split_h5.create_dataset('image_features', (len(split_roidb), 1024, 63, 63)) # image size 1000 -> feature map is 63
        split_spatial = split_h5.create_dataset('image_spatial', (len(split_roidb), 2))
        ...
        for i, each_item in tqdm(enumerate(split_roidb)):
            ...
            base_feat_np = base_feat.data.cpu().numpy()
            base_feat_height, base_feat_width = base_feat_np.shape[2], base_feat_np.shape[3]

            split_feat[i, :, :base_feat_height, :base_feat_width] = base_feat_np
            split_spatial[i] = np.array([base_feat_height, base_feat_width])
