...
def filter_answers(answers_dset, min_occurence):
    ...
    for answer in tqdm(occurence.keys()):
        if len(occurence[answer]) >= min_occurence:
            filted_occurence[answer] = occurence[answer]

    print('Num of answers that appear >= %d times: %d -> %d' % (
        min_occurence, len(occurence), len(filted_occurence)))
    return filted_occurence


def create_ans2label(occurence, name, cache_root='vqa_data/cache'):
...
def compute_target(answers_dset, ans2label, name, cache_root='vqa_data/cache'):
...
if __name__ == '__main__':
    train_answer_file = 'vqa_data/v2_mscoco_train2014_annotations.json'
    train_answers = json.load(open(train_answer_file))['annotations']

    val_answer_file = 'vqa_data/v2_mscoco_val2014_annotations.json'
    val_answers = json.load(open(val_answer_file))['annotations']

    train_question_file = 'vqa_data/v2_OpenEnded_mscoco_train2014_questions.json'
    train_questions = json.load(open(train_question_file))['questions']

    val_question_file = 'vqa_data/v2_OpenEnded_mscoco_val2014_questions.json'
